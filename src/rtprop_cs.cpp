...
/**
 * @file rtprop_cs.cpp Real time propagation for the Gross-Pitaevskii equation (cylinder symmetry)
 * @brief Real time propagation for the Gross-Pitaevskii equation (cylinder symmetry)
 * @author Želimir Marojević
 */
/** @page rtprop2 Real time propagation with cylinder symmetry
 * This is exactly doing the same as ...
 * The only differece is the inner product.
 * no 3d
 */
...
///namespace realtime_propagation_cs
namespace realtime_propagation_cs
{
  using namespace std;
  ...
  double MySolver<dim>::Particle_Number( LA::MPI::Vector &vec )
  {
    ...
    for (; cell != endc; ++cell)
    {
      if ( cell->is_locally_owned() )
      {
        ...
        {
          tmp1 += fe_values.JxW(qp) * fabs(fe_values.quadrature_point(qp)[1]) * (vec_vals[qp][0] * vec_vals[qp][0] + vec_vals[qp][1] * vec_vals[qp][1]);
        }
      }
    }
   ...
   * if \f$ r \neq 0\f$ then set boundary id to one.
   * Setting the boundary ids is important
   */
  template<int dim>
  ...
  void MySolver<dim>::Expectation_value_position( LA::MPI::Vector &vec, double *retval )
  {
    ...
    for (; cell != endc; ++cell)
    {
      if ( cell->is_locally_owned() )
      {
        ...
        {
          JxWxn = fe_values.JxW(q_point) * fabs(fe_values.quadrature_point(q_point)[1]) * (vec_vals[q_point][0] * vec_vals[q_point][0] + vec_vals[q_point][1] * vec_vals[q_point][1]);
  ...
  void MySolver<dim>::run()
  {
    ...
    pcout << "t == " << m_t << endl;
    pcout << "N == " << N << endl;
    pcout << "p == " << p[0] / N << ", " << p[1] / N << endl;
    pcout << "pos == " << pos[0] / N << ", " << pos[1] / N << endl;
    ...
    for ( unsigned i = 1; i <= m_NA; i++ )
    {
      ...
      m_table.insert( cols, MyTable::time, m_t );
      m_table.insert( cols, MyTable::ev_position_x, pos[0] / N );
      m_table.insert( cols, MyTable::ev_position_y, pos[1] / N );
      m_table.insert( cols, MyTable::ev_momentum_x, p[0] / N );
      m_table.insert( cols, MyTable::ev_momentum_y, p[1] / N );

      m_table.dump_2_file( "rt_prop_log.csv" );

      pcout << "N == " << N << endl;
      pcout << "pos == " << pos[0] / N << ", " << pos[1] / N << endl;
      pcout << "momentum == " << p[0] / N << ", " << p[1] / N << endl;
...
int main ( int argc, char *argv[] )
{
  ...
  {
    realtime_propagation_cs::MySolver<2> solver("params.xml");
    solver.run();
  }
  return EXIT_SUCCESS;
}
