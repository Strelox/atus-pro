...
* @brief This program solves the stationary Gross--Pitaevskii equation for a complex wave function in 2D or 3D.
*/
  ...
  void MySolver<dim>::assemble_system ()
  {
    ...
    for ( ; cell != endc; cell++ )
    {
      if ( cell->is_locally_owned() )
      {
        ...
        {
          double JxW = fe_values.JxW(qp);
          double fak = m_gs * Psi_ref[qp][0] * Psi_ref[qp][1];
          double Pot = Potential.value(fe_values.quadrature_point(qp)) - m_mu;
          double req = Psi_ref[qp][0] * Psi_ref[qp][0];
          double imq = Psi_ref[qp][1] * Psi_ref[qp][1];
          ...
          for ( unsigned i = 0; i < dofs_per_cell; i++ )
            for ( unsigned j = 0; j < dofs_per_cell; j++ )
              cell_matrix(i, j) += JxW * (fe_values[rt].gradient(i, qp) * fe_values[rt].gradient(j, qp) + (Pot + m_gs * (3 * req + imq)) * fe_values[rt].value(i, qp) * fe_values[rt].value(j, qp)
                                          + fak * fe_values[rt].value(i, qp) * fe_values[it].value(j, qp) + fak * fe_values[it].value(i, qp) * fe_values[rt].value(j, qp)
