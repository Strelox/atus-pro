...
#include "CBase.h"
    ...
    for ( ; cell != endc; cell++ )
    {
      if ( cell->is_locally_owned() )
      {
        ...
        for ( unsigned qp = 0; qp < n_q_points; qp++ )
        {
          ...
          Q1 = Potential.value(fe_values.quadrature_point(qp)) - m_mu + m_gs[0] * Psi_ref[qp] * Psi_ref[qp];
  ...
  void MySolver<dim>::make_grid_custom ()
  {
    ...
    CPotential<dim> Potential_fct ( m_omega, m_QN1[2] );
  ...
  void MySolver<dim>::run2b ()
  {
    ...
    for ( unsigned i = 0; i < m_Ndmu; i++ )
    {
      ...
      // nehari
      //m_ti = sqrt((m_mu*N-T)/(4.0*m_gs[0]*W)); // if m_Psi_2 == m_Psi_1
...
int main ( int argc, char *argv[] )
{
  ...
  {
    BreedSolver_cs::MySolver<2> solver("params.xml");
    solver.run2b ();
  }
  return EXIT_SUCCESS;
}